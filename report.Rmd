...
# calculate values for current KPO4 and how it compares with Scotland and target
  kpo_data1 <- kpo_data %>% filter(`Tracking Link` == "Total" & `Financial Year` == fin_yr)
  ...
  #Store value for previous financial year
     KPO4_prev <- all_kpo_data %>% filter(`Tracking Link` == "Total" & `Financial Year` == prev_fin_yr)
...
# text for when there is only 1 financial year
     text_kpo <- paste0("This indicator summarises performance across all questions, with differential weightings based on importance. For ", council_fltr," in ",fin_yr, " overall
performance is at ", KPO4_ytd, " for the year to date. ", "This is ",abbel_kpo4,
" the Scotland average of ", scotAv_kpo4," and ", hilow_kpo4," the target value of 7.5.")
     
    # text for when there are more than 1 financial years     
text_multiple_kpo <- paste0("This indicator summarises performance across all questions, with differential weightings based on importance. For ", la," in ",fin_yr, " overall
...
" The year to date performance of ", la, " in ", fin_yr, " is ", abbel_kpo4,
" the Scotland average of ", scotAv_kpo4," and ", hilow_kpo4," the target value of 7.5.")
...
ggplot(data = kpo_data1,
  ...
  scale_fill_manual(
    values = rep(c("cadetblue3", "dimgrey"),Years),
    name = ""
    )+
...
# filter to quarters, selected council and current financial year
     ...
     kpo_data1$`Tracking Link` <- factor(kpo_data1$`Tracking Link`, levels = QLabels)
     ...
     second_Q <- if(length(QLabels) > 1)
                    {kpo_data1$`Tracking Link`[[2]]} else
                    {0}
     third_Q <- if(length(QLabels) > 2) 
     {kpo_data1$`Tracking Link`[[3]]} else
         {0}
     fourth_Q <- if(length(QLabels) >3)
       {kpo_data1$`Tracking Link`[[4]]} else
       {0}
    
     #filter to get KPO for quarter 1
     Q1_kpo <- kpo_data1 %>% filter(`Tracking Link` == first_Q) %>%
       select(KPO_score)
     ...
     Q2_kpo <- kpo_data1 %>% filter(`Tracking Link` == second_Q) %>%
       select(KPO_score)
     ...
     Q3_kpo <- kpo_data1 %>% filter(`Tracking Link` == third_Q) %>%
       select(KPO_score)
     ...
     Q4_kpo <- kpo_data1 %>% filter(`Tracking Link` == fourth_Q) %>%
       select(KPO_score)
...
`r ifelse(length(unique(extra_data$`Financial Year`)) > 1, paste(main_text, extra_text), main_text)`
...
# Add Financial year to quarter labels
 line_data$`Tracking Link` <- gsub("Quarter\\ ","Q",line_data$`Tracking Link`, perl = T)
 line_data$Label <- paste(line_data$`Tracking Link`, line_data$`Financial Year`, sep = " ")
 line_data$`Tracking Link` <- gsub("Q", "", line_data$`Tracking Link`, perl = T)
...
```{r, eval = length(line_data$LA[line_data$LA == la]) > 1, echo = F}
...
qrtr_available <- line_data$`Tracking Link`[report_line_data$LA == la]
year_available <- line_data$`Financial Year`[report_line_data$LA == la]
