...
Respondents were asked to provide details on the type of respondent they were, as well as their reason for contacting the Building Standards Service in `r la`. Of the `r resp_number` respondents in, `r year`, `r discuss_perc` contacted the local authority to discuss their proposal before applying for a building warrant, `r appli_perc` were making an application for a warrant and `r constr_perc` contacted the service during construction. `r other_perc` contacted the service for some other reason.  
...
```{r echo = FALSE, comment = ""}
# Store the number of financial years available for council
Years <- kpo_data %>% filter(id == council_fltr)
Years <- length(unique(Years$`Financial Year`))
...
extra_data <- extra_data %>% 
  filter(`Tracking Link` == crnt_qtr, id == council_fltr)
...
```
In the `r year` year to date for the question "Thinking of your engagement, how satisfied were you with the time taken to complete the process?" responses for `r la` have been `r tm$pos_or_neg` with `r tm$total_good` percent saying that they were very satisfied or satisfied. The greatest proportion of respondents said they were `r tm$max_name` at `r tm$max_perc` This was followed by `r tm$sec_name` at `r tm$sec_perc` For Scotland overall, most respondents said that they were `r tm$scot_max_name` at `r tm$scot_max_perc`  
...
ggplot(data = time_data,
  ...
  scale_fill_manual(values = c("cadetblue3", "dimgrey"), name = "") +
  scale_y_continuous(expand = expansion(mult = c(0, 0.1))) +
  ggtitle(paste("Satisfaction with time taken - Year to Date", year)) +
  xlab("Responses") +
  ylab(paste("Percentage of Responses", year)) +
  theme_classic()
```
\newpage
...
```
In the `r year` year to date for the question "How would you rate the standard of communication provided?" responses for `r la` have been `r cm_txt$pos_or_neg` with `r cm_txt$total_good` percent saying that it was good or very good. The greatest proportion of respondents said they felt it was `r cm_txt$max_name` at `r cm_txt$max_perc` This was followed by `r cm_txt$sec_name` at `r cm_txt$sec_perc` For Scotland overall, most respondents said that communication was `r cm_txt$scot_max_name` at `r cm_txt$scot_max_perc`  
...
ggplot(data = comms_data,
  ...
  scale_fill_manual(values = c("cadetblue3", "dimgrey"), name = "") +
  scale_y_continuous(expand = expansion(mult = c(0, 0.1))) +
  ggtitle(paste("Standard of Communication - Year to Date", year)) +
  xlab("Responses") +
  ylab(paste("Percentage of Responses", year)) +
  theme_classic()
```
...
ggplot(data = info_data,
  ...
  scale_fill_manual(values = c("cadetblue3","dimgrey"), name = "") +
  ggtitle(paste("Quality of information - Year to Date", year)) +
  xlab("Responses") +
  scale_y_continuous(expand = expansion(mult = c(0, 0.1))) +
  ylab(paste("Percentage of Responses", year)) +
  theme_classic()
...
```
In the `r year` year to date for the question "How would you rate the service offered by staff?" responses for `r la` have been `r staff_txt$pos_or_neg` with `r staff_txt$total_good` percent saying that it was good or very good. The greatest proportion of respondents said they felt it was `r staff_txt$max_name` at `r staff_txt$max_perc` This was followed by `r staff_txt$sec_name` at `r staff_txt$sec_perc` For Scotland overall, most respondents said that the service received was `r staff_txt$scot_max_name` at `r staff_txt$scot_max_perc`  
...
ggplot(data = staff_data, 
  ...
  scale_fill_manual(values = c("cadetblue3", "dimgrey"), name = "") +
  scale_y_continuous(expand = expansion(mult = c(0, 0.1))) +
  ggtitle(paste("Service offered by staff - Year to Date", year)) +
  xlab("Responses") +
  ylab(paste("Percentage of Responses", year)) +
  theme_classic()
```
...
ggplot(data = responsive_data,
  ...
  scale_fill_manual(values = c("cadetblue3", "dimgrey"), name = "") +
  ggtitle(paste("Responsiveness to queries or issues - Year to Date", year))+
  scale_y_continuous(expand = expansion(mult = c(0, 0.1))) +
  xlab("Responses") +
  ylab(paste("Percentage of Responses", year)) +
  theme_classic()
...
ggplot(data = fair_data,
  ...
  scale_fill_manual(values = c("cadetblue3", "dimgrey"), name = "") +
  ggtitle(paste("Would you agree you were treated fairly - Year to Date",
                year)) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.1))) +
  xlab("Responses") +
  ylab(paste("Percentage of Responses", year)) +
...
```
In the `r year` year to date for the question "Overall, how satisfied were you with the service provided?" responses for `r la` have been `r ovr_txt$pos_or_neg` with `r ovr_txt$total_good` percent saying that they were very satisfied or satisfied. The greatest proportion of respondents said they were `r ovr_txt$max_name` at `r ovr_txt$max_perc` This was followed by `r ovr_txt$sec_name` at `r ovr_txt$sec_perc` For Scotland overall, most respondents said that they were `r ovr_txt$scot_max_name` at `r ovr_txt$scot_max_perc`  
...
ggplot(data = overall_data,
  ...
  scale_fill_manual(values = c("cadetblue3", "dimgrey"), name = "") +
  scale_y_continuous(expand = expansion(mult = c(0, 0.1))) +
  ggtitle(paste("Overall satisfaction - Year to Date", year)) +
  xlab("Responses") +
  ylab(paste("Percentage of Responses", year)) +
  theme_classic()
```
