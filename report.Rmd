...
kpo_data <- kpo_data %>% filter(`Tracking Link` == "Total")
ggplot(data = kpo_data,
            aes(
         x = `Tracking Link`, 
         y = KPO_score, 
         fill = id)
...
local_auth <- "Aberdeen City" ##will need to select LA based on log in details
     ...
     KPO4_ytd <- kpo_data %>% filter(id == "local authority") %>% pull(KPO_score)
     hilow_kpo4 <- ifelse(KPO4_ytd > 7.5, "higher", "lower")
     scotAv_kpo4 <- kpo_data %>% filter(id == "Scotland") %>% pull(KPO_score)
...
```
In this year for `r la` to date for the question "How would you rate the service offered by staff?" responses have been `r inf_txt$pos_or_neg` with `r inf_txt$total_good` percent saying that it was good or very good. The greatest proportion of respondents said they felt it was `r inf_txt$max_name` at `r inf_txt$max_perc` This was followed by `r inf_txt$sec_name` at `r inf_txt$sec_perc` For Scotland overall, most respondents said that the service received was `r inf_txt$scot_max_name` at `r inf_txt$scot_max_perc`
