...
kpo_data_ytd <- kpo_data %>%
  filter(Quarter == "Year to Date" & `Financial Year` == year)

# Council KPO4
KPO4_ytd <- kpo_data_ytd  %>% 
  filter(id == la) %>% 
  pull(KPO_score)
# Scotland KPO4
scotAv_kpo4 <- kpo_data_ytd  %>% 
  filter(id == "Scotland") %>% 
  pull(KPO_score)
...
agent_perc <- typ_dta_filter %>%
      filter(Question == "Agent/Designer" & value == 1) %>%
      pull(perc)
appli_perc <- typ_dta_filter %>%
      filter(Question == "Applicant" & value == 1) %>%
      pull(perc)
contr_perc <- typ_dta_filter %>%
      filter(Question == "Contractor" & value == 1) %>%
      pull(perc)
type_other_perc <- typ_dta_filter %>%
...
QLabels <- unique(kpo_data1$Quarter)
kpo_data1$Quarter <- factor(kpo_data1$Quarter, 
                                    levels = QLabels
                                    )
...
# Number of quarters
no_qrtrs <- length(extra_data$Quarter)
...
line_data$Quarter <- gsub("Quarter\\ ",
                                  "Q",
                                  line_data$Quarter, 
                                  perl = TRUE
                                  )
line_data$Label <- paste(line_data$Quarter, 
                         line_data$`Financial Year`, 
                         sep = " "
                         )
line_data$Quarter <- gsub("Q",
                                  "",
                                  line_data$Quarter, 
                                  perl = TRUE
                                  )
...
line_data <- arrange(line_data, `Financial Year`, Quarter, LA)
...
qrtr_available <- line_data$Quarter[line_data$LA == la]
